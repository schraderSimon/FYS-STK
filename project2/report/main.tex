...
\begin{tikzpicture}[thick,scale=0.6, every node/.style={scale=0.6}][shorten >=1pt,->,draw=black!50, node distance=\layersep]
    ...
    \foreach \name / \y in {0,1,2}
        \path[yshift=-1.5cm]
            node[output neuron] (O-\name) at (5*\layersepSmall,-\y cm) {$\y$};
    
    \path[yshift=-0.3cm]
            node[missing] (O-4) at (5*\layersepSmall,-4 cm) {};
    
    \path[yshift=-0.1cm]
            node[output neuron] (O-5) at (5*\layersepSmall,-5 cm) {$9$};
    %Draw empty neuron for x- seperation   
    \path[yshift=-0.1cm]
            node[dead] (D-5) at (6*\layersepSmall,-5 cm) {};
...
\caption[NN Hidden layer Activation]{}\label{NN_activation}
\end{figure}
...
\caption[Classification accuracy to learning rate (Softmax Regression)]{Multinomial logistic regression with stochastic gradient descent. Classification accuracy on the test set as a function of learning rate. Using 150 epochs.}\label{Acc_to_LR}
\end{figure}

\subsubsection{Comparing Implementation to Scikit- learn}
Using a learning rate $\eta = 0.0001$ and $500$ epochs on $6$ independent runs. Comparing the Scikit- learn implementation with our own implementation of Logistic regression with Stochastic regression we get 
\begin{table}[]
\caption[Classification accuracy comparison to Scikit- Learn]{Own implementation of Softmax regression with stochastic gradient descent compared to a Scikit- Learn implementation using the LBFGS. Classification accuracy on a test set randomized between each run. Using 500 epochs for both methods and a learning rate of 0.0001 for the SGD solver.}
\begin{tabular}{|l|l|l|}
\hline
