...
def neural_network(input,regulizer,learningrate,type,layers=[100,100]):
    model=keras.models.Sequential()
    #model.add(norm)
    model.add(layers.Dense(layers[0],activation=type,kernel_regularizer=regulizer,input_dim=input.shape[1]))
    for i in range(1,len(layers)):
        model.add(layers.Dense(layers[i],kernel_regularizer=regulizer,kernel_initializer='normal',bias_initializer="random_normal",activation=type))
