...
""" OUTPUT CSV """
dict = {'polynomial degree': [i for i in range(1,maxdeg+1)], 'OLS Max std. dev.':  np.round(Z_table*std_dev_ridge[:,0],2), 'OLS_beta_max':  max_beta[:,0], 'OLS_beta_min':  min_beta[:,0],
'Ridge Max std. dev. (lambda = {0})'.format(lambda_val_ridge[1]): np.round(Z_table*std_dev_ridge[:,1],2), 'Ridge_beta_max(lambda = {0})'.format(lambda_val_ridge[1]):  max_beta[:,1],'Ridge_beta_max(lambda = {0})'.format(lambda_val_ridge[1]):  min_beta[:,1],'Ridge Max std. dev. (lambda = {0})'.format(lambda_val_ridge[2]): np.round(Z_table*std_dev_ridge[:,2],2), 'Ridge_beta_max(lambda = {0})'.format(lambda_val_ridge[2]):  max_beta[:,2], 'Ridge_beta_max(lambda = {0})'.format(lambda_val_ridge[2]):  min_beta[:,2]}
